...
actor TestResourceManager {
    ...
    private func downloadResources() async throws -> (URL, URL) {
        // Check for local model in test bundle
        // Note: .process("Resources") flattens the directory structure, so files from
        // Resources/Model/ are copied directly to the bundle root
